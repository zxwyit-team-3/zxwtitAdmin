...
export default {
  ...
  methods: {
    ...
    compiles() {
      var _then = this;
      console.log(_then.value.userTypeId);
      _then.axios
        .post(
          "/api/UserType/ModifyUserRole?id=" +
            this.value.userTypeId +
            "&userRoleName=" +
            _then.userUpdata
        )
        .then(res => {
    ...
    increases(){
      var _then = this;
      _then.axios.post("/api/UserType/AddUserRole?userRoleName=" + _then.user).then(res=>{
        _then.apply();
